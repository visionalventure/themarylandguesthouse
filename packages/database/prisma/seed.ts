...
async function main() {
  ...
  for (const guest of guestData) {
    ...
    const tier = guest.totalStays >= 10 ? 'PLATINUM' : guest.totalStays >= 5 ? 'GOLD' : guest.totalStays >= 3 ? 'SILVER' : 'BRONZE';
  ...
  for (let i = 1; i <= 10; i++) {
    await prisma.restaurantTable.upsert({
      where: { restaurantId_tableNumber: { restaurantId: restaurant.id, tableNumber: `T${i.toString().padStart(2, '0')}` } },
      update: {},
      create: {
  ...
  for (const item of menuItems) {
    await prisma.menuItem.upsert({
      where: { id: `menu-${item.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}` },
      update: {},
      create: {
        id: `menu-${item.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}`,
        restaurantId: restaurant.id,
  ...
  console.log('🍽️  Restaurant: Maryland Dining with 10 tables & 8 menu items');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
}
